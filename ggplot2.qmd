...
As always, there are other packages that can achieve this (e.g. `patchwork`), but `cowplot` is a good choice for its simplicity and ease of use.
...
- Plot is self-contained: the plot should be understandable without needing to refer to the main text. Achieve this by including a caption with an informative title, using sentences to describe any important details, and closing with abbreviations or symbols used in the plot.

## Captions and cross-referencing

I outline how to create captions and cross-references for tables in the `Tables` section.
