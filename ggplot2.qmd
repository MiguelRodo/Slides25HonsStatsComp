...
- The **`plot_grid()`** function accepts a vector of plots (here, `p1` and `p2`).
- Use the **`labels`** argument to automatically add subplot labels.
- The **`align`** parameter ensures that the axes are aligned (set to `"hv"` for both horizontal and vertical alignment).
- With **`theme_cowplot()`** (from the cowplot package), you can set a consistent base font size and styling across all plots, which helps avoid the common pitfall of tiny text elements.

When you incorporate these combined plots in your Quarto document, be sure to assign a figure caption (using the `fig-cap` attribute) and a label (like `fig-combined`) so that you can reference the entire figure later using the `@fig-combined` syntax.
