...
3. Explain why the standard deviation is `NA` for one carrier, and why it is `0` for others.
...
6. Using the `airlines` and `flights` datasets, find the names of the airlines with the worst and best average arrival delay in the month of July. From this information, identify the route with the greatest difference between the best and worst performing airlines. Discover whether this is a short or a long route.
