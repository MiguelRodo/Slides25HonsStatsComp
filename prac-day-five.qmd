...
6. Using the `airlines` and `flights` datasets, do the following:
...
- Investigate factors from the `flights` dataset that could have contributed to this difference
...
route_info_diff
# quite a short time (0.3), not much gain,
# low departure delay but larger arriveal delay,
# quite weird, honestly... two hours!
route_info_diff |>
  dplyr::mutate(
    distance = ecdf(route_info$distance)(distance),
    time = ecdf(route_info$time)(time),
    speed = ecdf(route_info$speed)(speed),
    gain = ecdf(route_info$gain)(gain),
