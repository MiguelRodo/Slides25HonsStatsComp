...
3. If you are working within Quarto, then you can use the Quarto cross-referencing syntax (details [here](https://quarto.org/docs/authoring/cross-references.html)).
...
Using `LaTeX`: If you are using `LaTex`, then you would run the `knitr::kable` command with `format = "latex"`.
