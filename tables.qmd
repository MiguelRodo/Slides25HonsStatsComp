...
Below is a sample Quarto code chunk that produces a table from an R data frame. In this example, we create a summary table of the penguins dataset, @tab-summary:

```{r}
#| echo: fenced
#| label: tab-summary
#| tab-cap: "Summary Statistics for Penguin Measurements. Values are rounded to 2 decimal places. Abbreviations: FL = Flipper Length (mm), BM = Body Mass (g)."
#| results: asis
...
- **Caption and Label:**  
  The code chunk header includes a `tab-cap` attribute that begins with `tab-summary:`. This sets the table’s caption and label. In your text, reference it as “@tab-summary.”
- **Table Formatting:**  
...
## Generating `LaTeX` Tables directly from R

If you have a `LaTeX` document, you can generate tables directly from R using `knitr::kable()` with the `format = "latex"` option. Here's an example:
...
The output above is then copied and pated into your `LaTeX` document.
